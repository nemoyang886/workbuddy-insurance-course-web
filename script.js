(function(){
  ...
  const stageList = [
    ...
    {end:35, label:"长期联系系统", color:"#1aa6d9"},
  ...
  const promptMeta = {
    ...
    32:["统一课堂案例","选择保险顾问版或通用版替换资料","一张长期联系快照"],
    33:["整理事实和联系分析","粘贴客户经营资料或对象沟通记录","一份关系温度和沟通分析"],
    34:["生成表达和行动计划","按关系温度安排联系动作","三套联系表达和7天计划"],
  ...
  function enhanceCover(){
    ...
    const visual = el("div","cover-visual ai-hero",`
      <figure class="cover-hero-card course-hero-card">
        ...
        <figcaption>WorkBuddy AI 工作流 · 资料 / Task / Skill / 长期联系系统</figcaption>
  ...
  function enhanceLearningPathOverview(){
    ...
    const labels = [
      ...
      ["Practice","最后实操","安装检查、界面认知、内容发布、长期联系。"],
  ...
  function enhanceIdentityChoice(){
    ...
    const items = [
      ["保险从业者","用朋友圈、长期联系、保单年检和复盘练习。"],
      ["客户/普通学员","用学习整理、日常沟通、家庭资料和个人事务练习。"],
      ["小龙虾新手","先照着模板跑通一件真实小事，再逐步升级。"]
    ];
  ...
  function addCatchUpNotes(){
    const notes = {
      ...
      30:"没有业务资料：用课堂统一案例练习。其他行业学员：把客户替换成需要持续联系的对象。",
  ...
  function enhanceCustomerPromptStructure(){
    [33,34].forEach((page) => {
      ...
      const structure = el("div","task-structure",["事实整理","关系温度","沟通分析","联系表达","7天计划","自检"].map((item) => `<span>${item}</span>`).join(""));
  ...
  function enhanceSevenDayPlan(){
    ...
    setActive(0);
    body.appendChild(el("div","action-closing","3 天只盯两个成果：一条满意朋友圈样本，一个跑通的长期联系案例。"));
  }
  ...
  function enhanceFollowupProfessional(){
    ...
    const panel = el("div","followup-principles",`
      <strong>长期联系的专业逻辑</strong>
      <span>保险行业叫定联，本质是服务提醒、关系维护和下一次关心，不是催促成交。</span>
      <div>
        <b>名单池</b>
        <b>关系温度</b>
        <b>价值触达</b>
    ...
    const visual = el("div","closing-office closing-finale",`
      <div class="closing-finale-card">
        ...
        <div class="closing-sprint">
          ...
          <div><span>Day 3</span><b>跑通长期联系案例</b></div>
        </div>
